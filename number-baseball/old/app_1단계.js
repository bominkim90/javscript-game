...
inputInterface.on("line", (number) => {
  if( viewingRecord ){ // 세부 기록 조회
    return detailRecord();
  }
  if( nowPlaying ){ // 게임중 O
    return nowPlayingLogic(number);
  } 
  if(number == 1) { // 게임 시작 (처음 프로그램 들어와서 '1')
    return startGame();
  } 
  if(number == 2) { // 전체 기록 보기
    viewingRecord = true;
    return viewRecord();
  }
  if(number == 3) { // 게임 통계 확인
...
function detailRecord(number){
  if(number.length == 1){
    ...
    if(totalRecords[parseInt(number)-1]){ // 게임 기록 조회
...
// 게임중 로직
function nowPlayingLogic(){
  if(oneGameRecord.endTime){ // 한판 종료된 후 => 재시작하기
